...
def tasks_routes(app, db):
	...
	@tasks_bp.get("/list_riddles")
	def list_riddles():
		'''Страница загадок'''
		lengs = Lengs.query.all()
		response = Template(title='Загадоки', info='Загадоки', subinfo='Выберите загадоку')
		return render_template('tasks.html', response=response, lengs=lengs) # quest=quest
	...
	def compile_task():
		...
		try:
			...
			if 'os' not in jn['code'] or 'eval' not in jn['code']:
				with open(f'compile/{ jn["name"] }.py', 'w') as f:
					f.write( jn['code'] )
			# else: raise ExceptionServer("Такой модуль не поддерживается")

			out = subp.run([ "python", f"compile/{ jn['name'] }.py" ], capture_output=True)
			error = out.stderr
			if error != b'':
				result = error.decode()
				valid = "failed"
			else:
				with open('compile/out.txt', 'r') as f: result = f.read().strip().split('\n')
			...
			print(os.listdir())
			os.remove(f'compile/{ jn["name"] }.py')
	...
	def compile_ruby():
		...
		try:
			...
			if 'os' not in jn['code'] or 'eval' not in jn['code']:
				with open(f'compile/{ jn["name"] }.rb', 'w') as f:
					f.write( jn['code'] )

			out = subp.run([ "ruby", f"compile/{ jn['name'] }.rb" ], capture_output=True)
			error = out.stderr
			...
			os.remove(f'compile/{ jn["name"] }.rb')
	...
	def compile_js():
		...
		try:
			...
			if 'os' not in jn['code'] or 'eval' not in jn['code']:
				with open(f'compile/{ jn["name"] }.js', 'w') as f:
					f.write( jn['code'] )

			out = subp.run([ "ruby", f"compile/{ jn['name'] }.js" ], capture_output=True) # node vmesto ruby
			error = out.stderr
			...
			os.remove(f'compile/{ jn["name"] }.js')
	...
	def compile_php():
		...
		try:
			...
			if 'os' not in jn['code'] or 'eval' not in jn['code']:
				with open(f'compile/{ jn["name"] }.php', 'w') as f:
					f.write( jn['code'].replace("include(\"./test.php\");", "function test($a=null, $b=null, $err=null) {\n\tfile_put_contents('compile/out.txt', $err != null ? $err : json_encode($a == $b) . \"\n\", FILE_APPEND);\n}\n\n\n") )

			out = subp.run([ "php", f"compile/{ jn['name'] }.php" ], capture_output=True)
			error = out.stderr
			...
			os.remove(f'compile/{ jn["name"] }.php')
	...
	def compile_type():
		...
		try:
			...
			if 'os' not in jn['code'] or 'eval' not in jn['code']:
				with open(f'compile/{ jn["name"] }.ts', 'w') as f:
					f.write( jn['code'] )

			out = subp.run([ "tsc", f"compile/{ jn['name'] }.ts" ], capture_output=True)
			error = out.stdout
			...
			if error != b'':
				...
				out = subp.run([ "node", f"compile/{ jn['name'] }.js" ], capture_output=True)
				error = out.stderr
			...
			os.remove(f'compile/{ jn["name"] }.ts')
			os.remove(f'compile/{ jn["name"] }.js')
